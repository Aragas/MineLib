        ...
        private void WorldBus_MessageReceived(object sender, MBusMessageReceivedEventArgs args)
        {
            IEnumerable<ChunkEnumerableResponse> GetChunksInCircleRequestEnumerable(ChunksInCircleRequest request)
            {
                ...
                foreach(var chunk in GetChunksInCircleRequest(request.X, request.Z, request.Radius))
                    yield return new ChunkEnumerableResponse() { GUID = request.GUID, Chunk = chunk };
            }
            ...
            InternalBus.HandleRequest<SectionRequest, SectionResponse>(InternalBus.WorldBus, args, request => new SectionResponse() { Section = WorldHandler.GetSection(request.Position) });
            InternalBus.HandleRequest<ChunkRequest, ChunkResponse>(InternalBus.WorldBus, args, request => new ChunkResponse() { Chunk = WorldHandler.GetChunk(request.Coordinates) });
            InternalBus.HandleRequest<ChunksInCircleRequest, ChunkBulkResponse>(InternalBus.WorldBus, args, request =>
            {
                if (request.SendBulk)
                    return new ChunkBulkResponse() { GUID = request.GUID, ChunkBulk = GetChunksInCircleRequest(request.X, request.Z, request.Radius).ToArray() };
                return null;
            });
            InternalBus.HandleRequest<ChunksInSquareRequest, ChunkBulkResponse>(InternalBus.WorldBus, args, request =>
            {
                if (request.SendBulk)
                    return new ChunkBulkResponse() { GUID = request.GUID, ChunkBulk = GetChunksInSquareRequest(request.X, request.Z, request.Radius).ToArray() };
                return null;
            });
