...
$result = pg_exec($Conn, "SELECT * FROM servers");
$Time = date('Y-m-d H:i:s');
if (!pg_exec($Conn, "DELETE FROM servers WHERE LastTimeSeen < NOW() - INTERVAL 2 MINUTE")) //delete old servers
...
<tbody>";
$Count = 0;
while ($row = pg_fetch_array($result,$Count))
{
    $Count++;
