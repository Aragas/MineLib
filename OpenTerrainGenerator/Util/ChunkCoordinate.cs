...
namespace OpenTerrainGenerator.Util
{
    ...
    public class ChunkCoordinate
    {
        ...
        private static int CHUNK_POPULATION_OFFSET_X = CHUNK_X_SIZE / 2 - 1;
        private static int CHUNK_POPULATION_OFFSET_Z = CHUNK_Z_SIZE / 2 - 1;
    ...
    public override bool Equals(Object otherObject)
        ...
        public static ChunkCoordinate getPopulatingChunk(int blockX, int blockZ)
        {
            ...
            return new ChunkCoordinate(
                    (int) Math.Floor((blockX - CHUNK_POPULATION_OFFSET_X) / (double)CHUNK_X_SIZE),
                    (int) Math.Floor((blockZ - CHUNK_POPULATION_OFFSET_Z) / (double)CHUNK_Z_SIZE));
        }
        ...
        {
            return new ChunkCoordinate(
                    (int) Math.Floor(blockX / (double)CHUNK_X_SIZE),
                    (int) Math.Floor(blockZ / (double)CHUNK_Z_SIZE));
        }
    ...
    public override String ToString()
         ...
         */
        public bool coordsMatch(int chunkX, int chunkZ)
        {
         ...
         * otherwise.
         */
        public bool populatesForBlock(int blockX, int blockZ)
        {
            return coordsMatch(
                    (int) Math.Floor((blockX - CHUNK_POPULATION_OFFSET_X) / (double)CHUNK_X_SIZE),
                    (int) Math.Floor((blockZ - CHUNK_POPULATION_OFFSET_Z) / (double)CHUNK_Z_SIZE));
