...
namespace PokeD.Server.PlayerBus.P3D
{
    public partial class P3DPlayer
    {
        ...
        private bool PokemonsValid(string pokemonData) =>
            //!Module.ValidatePokemons ||
            !IsOfficialGameMode || new DataItems(pokemonData).DataItemsToMonsters().All(pokemon => pokemon.IsValid);
        private bool PokemonValid(string pokemonData) =>
            //!Module.ValidatePokemons ||
            !IsOfficialGameMode ||
            new Monster(pokemonData).IsValid;
        ...
        private void HandleTradeOffer(TradeOfferPacket packet)
        {
            ...
            if (PokemonValid(packet.TradeData))
                Module.OnTradeRequest(this,  packet.TradeData, destClient);
            else
            {
